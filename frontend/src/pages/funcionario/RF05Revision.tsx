import {
  IonContent, IonPage,
  IonButton, IonIcon,
  IonItem, IonSelect, IonSelectOption, IonTextarea, useIonRouter, useIonViewWillEnter
} from '@ionic/react';
import {
  documentTextOutline, imageOutline,
  checkmarkOutline, alertCircle
} from 'ionicons/icons';
...
const RF05Revision: React.FC = () => {
  ...
  useIonViewWillEnter(() => {
    void (async () => {
      ...
      if (id) {
        const t = await tramitesService.getById(id);
        if (t) {
          setTramite(t);
  ...
  const handleGuardar = async () => {
    ...
    await tramitesService.updateEstado(tramite.id, nuevoEstado, actor, observacion || undefined);
  ...
  const getDocumentos = () => {
    if (!tramite) return [];
    const docs: Array<{ nombre: string; icono: string; tipo: 'original' | 'correccion' | 'none' }> = [];
    ...
    if (docs.length === 0) {
      docs.push({ nombre: 'Sin documentos adjuntos', icono: documentTextOutline, tipo: 'none' });
    }
    return docs;
  };
              ...
              <div style={{ padding: '20px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px' }}>
                <div>
                  ...
                  <p style={{ margin: 0, color: '#4A4A4A', fontSize: '.92rem' }}>
                    Trámite <strong>{tramite.ticket}</strong> · {tramitesService.getTipoLabel(tramite.tipo)} · Ciudadano: {tramite.ciudadanoNombre}
                  </p>
                </div>
            ...
            {guardado ? (
              <div style={{ background: 'white', borderRadius: '10px', boxShadow: '0 6px 18px rgba(10,19,45,.08)', padding: '40px', textAlign: 'center' }}>
                ...
                <p style={{ margin: '0 0 20px', color: '#4A4A4A', fontSize: '.92rem' }}>
                  El trámite <strong>{tramite.ticket}</strong> fue marcado como <strong>{estadoRevision === 'aprobar' ? 'Aprobado' : estadoRevision === 'rechazar' ? 'Rechazado' : 'Observado'}</strong>.
                      ...
                      <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px', padding: '.75rem 0', borderBottom: i < getDocumentos().length - 1 ? '1px solid #e8eef5' : 'none' }}>
                        ...
                        {doc.tipo !== 'none' && (
                          <IonButton
                            ...
                            size="small"
                            onClick={() => {
                              if (doc.tipo === 'original' && tramite?.archivoNombre) {
                                tramitesService.downloadArchivo(tramite.id, tramite.archivoNombre);
                              } else if (doc.tipo === 'correccion' && tramite?.archivoCorreccionNombre) {
                                tramitesService.downloadArchivoCorreccion(tramite.id, tramite.archivoCorreccionNombre);
                              }
                            }}
