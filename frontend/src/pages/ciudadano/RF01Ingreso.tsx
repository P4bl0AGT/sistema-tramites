  ...
  const handleSend = async () => {
    ...
    try {
      const tramite = await tramitesService.create({
        ...
        archivo:         tramiteData.archivo,
        ciudadanoId:     String(user.id),
        ciudadanoNombre: user.nombre,
        ciudadanoEmail:  user.correo,
      });
      setTicketGenerado(tramite.ticket);
      setEnviado(true);
    } catch (err: unknown) {
      const e = err as { response?: { data?: { error?: string } } };
      setErrorEnvio(e?.response?.data?.error ?? 'Error al enviar el trámite. Intenta de nuevo.');
    } finally {
      setEnviando(false);
    }
  };
                ...
                <div style={{ padding: '20px' }}>
                  ...
                  {errorEnvio && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: '#ffe9e9', border: '1px solid #f5b8b8', borderRadius: '6px', padding: '10px 14px', marginBottom: '16px' }}>
                      <IonIcon icon={informationCircleOutline} style={{ color: '#8b1f1f', fontSize: '18px', flexShrink: 0 }} />
                      <span style={{ fontSize: '.86rem', color: '#8b1f1f' }}>{errorEnvio}</span>
                    </div>
                  )}

                  <IonButton expand="block" onClick={handleSend} disabled={!isFormValid || enviando}
