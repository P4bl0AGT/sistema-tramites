...
const TIMELINE_LABELS = [
  ...
  { label: 'Notificación', sub: (_: Tramite) => 'Pendiente' },
];
    ...
    <IonPage>
                  ...
                  {[...tramites].reverse().map((t, i) => {
                    ...
                    return (
                      <div key={t.id} onClick={() => setSelectedId(t.id)} style={{
                        ...
                        <strong style={{ display: 'block', fontSize: '.86rem', color: '#0A132D' }}>{t.id}</strong>
                  ...
                  <div>
                    ...
                    <div style={{ background: 'white', borderRadius: '10px', boxShadow: '0 6px 18px rgba(10,19,45,.08)', padding: '20px 24px', marginBottom: '16px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '10px', marginBottom: '24px' }}>
                        <div>
                          <h3 style={{ margin: '0 0 4px', color: '#0A132D', fontWeight: 700, fontSize: '1rem' }}>Ticket {selected.id}</h3>
      ...
      <IonFooter className="ion-no-border">
        <IonTabBar slot="bottom" style={{ borderTop: '1px solid #ddd' }}>
          <IonTabButton tab="tramite" routerLink="/ciudadano/ingreso"><IonIcon icon={addCircleOutline} /><IonLabel>Trámite</IonLabel></IonTabButton>
          <IonTabButton tab="estado" selected style={{ '--color-selected': 'var(--gob-primary)' }}><IonIcon icon={listOutline} /><IonLabel>Estado</IonLabel></IonTabButton>
          <IonTabButton tab="avisos" onClick={() => router.push('/ciudadano/notificaciones', 'forward', 'push')}><IonIcon icon={chatbubbleEllipsesOutline} /><IonLabel>Avisos</IonLabel></IonTabButton>
        </IonTabBar>
      </IonFooter>
    </IonPage>
  );
