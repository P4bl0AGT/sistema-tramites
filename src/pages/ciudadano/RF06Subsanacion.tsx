import {
  ...
  IonItem, IonLabel, IonTextarea, useIonRouter, useIonViewWillEnter
} from '@ionic/react';
import {
...
const RF06Subsanacion: React.FC = () => {
  ...
  useIonViewWillEnter(() => {
    ...
    if (email) {
      ...
      if (observados.length > 0) {
        setTramite(observados[observados.length - 1]);
      } else {
        setTramite(null);
      }
    }
  ...
  const handleSubsanar = () => {
    ...
    tramitesService.subsanar(tramite.id, actor, archivo?.name, comentario || undefined);
              ...
              <div style={{ padding: '20px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px' }}>
                <div>
                  ...
                  <p style={{ margin: 0, color: '#4A4A4A', fontSize: '.92rem' }}>
                    Trámite <strong>{tramite.id}</strong> · Revisa el motivo de la observación y sube solo los documentos corregidos.
                  </p>
                </div>
            ...
            {enviado ? (
              <div style={{ background: 'white', borderRadius: '10px', boxShadow: '0 6px 18px rgba(10,19,45,.08)', padding: '40px', textAlign: 'center' }}>
                ...
                <p style={{ margin: '0 0 20px', color: '#4A4A4A', fontSize: '.92rem' }}>
                  El trámite <strong>{tramite.id}</strong> volvió a estado <strong>En revisión</strong>.
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '16px', marginBottom: '16px', alignItems: 'start' }}>
                  ...
                  <div style={{ background: 'white', borderRadius: '10px', boxShadow: '0 6px 18px rgba(10,19,45,.08)', overflow: 'hidden' }}>
                    ...
                    <div style={{ padding: '16px' }}>
                      ...
                      <p style={{ margin: '0 0 6px', fontSize: '.86rem', color: '#4A4A4A' }}><strong>Ticket:</strong> {tramite.id}</p>
