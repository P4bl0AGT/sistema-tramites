    ...
    t.historial.forEach((h, i) => {
      ...
      switch (h.estado) {
        case 'recepcionado':
          ...
          cuerpo    = `El trámite ${t.id} · ${tipoLabel} fue recibido por la oficina de partes y está siendo procesado. No se requiere ninguna acción por tu parte. Te notificaremos ante cualquier cambio de estado.`;
          break;
        case 'observado':
          titulo    = 'Trámite observado';
          pillLabel = 'Observado';
          h3        = 'Tu trámite requiere una corrección';
          cuerpo    = `El trámite ${t.id} · ${tipoLabel} cambió de estado a Observado. Acción requerida: Ingresa al portal y sube únicamente el documento corregido antes del vencimiento del plazo legal.`;
        ...
        case 'aprobado':
          ...
          cuerpo    = `El trámite ${t.id} · ${tipoLabel} fue aprobado por la unidad correspondiente. Puedes consultar el detalle en el portal de trazabilidad.`;
          break;
        case 'rechazado':
          titulo    = 'Solicitud rechazada';
          pillLabel = 'Rechazado';
          h3        = 'Tu solicitud fue rechazada';
          cuerpo    = `El trámite ${t.id} · ${tipoLabel} fue rechazado. Puedes consultar el motivo detallado en el portal de trazabilidad.`;
          break;
      }

      notifs.push({
        id:         `${t.id}_${h.estado}_${i}`,
        tramiteId:  t.id,
        tipoLabel,
        titulo,
        fecha:      h.fecha,
...
const RF07Notificaciones: React.FC = () => {
  ...
  useIonViewWillEnter(() => {
    ...
    if (!email) return;
    const tramites = tramitesService.getByUser(email);
                ...
                {selected && (
                  <div style={{ border: '1px solid #d7dee8', borderRadius: '10px', overflow: 'hidden', background: 'white', boxShadow: '0 6px 18px rgba(10,19,45,.06)' }}>
                    <div style={{ background: '#f1f5f9', padding: '.85rem 1rem', borderBottom: '1px solid #d7dee8', fontSize: '.84rem', color: '#4A4A4A', lineHeight: 1.6 }}>
                      ...
                      <div><strong>Asunto:</strong> Cambio de estado del trámite {selected.tramiteId}</div>
                    </div>
